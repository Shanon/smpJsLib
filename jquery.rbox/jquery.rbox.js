...
(function ($) {
    ...
    $.fn[name_space] = function(options) {
        ...
        *  グルーピングされたクラス以下のinputをキャッシュする
        *  キャッシュするキーは複数クラスを利用している可能性を考慮して、クラス名
        *  から空白値を取り除いた値
        */
        // -------------------------------------------------------------------------- //
        $OBJ.each(function(index) {
            ...
            // キャッシュのキーはクラス名から空白を除いた文字列(複数クラス対応)
        ...
        *
        *  自分の親(セレクタのnodeName(クラス名rbox)まで)以下の自分以外の
        *  チェックボックスのチェックをすべてとる。
        *
        *  という処理
        */
        ...
        $OBJ.find("input").click(function() {
            ...
            if($thisInput.attr("checked") === 'checked') {
                ...
                // キャッシュキーを使って、自分の所属するグループのキャッシュを呼ぶ
                // 自分を除くinputのcheckedをfalseにする
